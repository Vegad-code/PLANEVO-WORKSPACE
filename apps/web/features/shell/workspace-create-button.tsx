...
export function WorkspaceCreateButton({ onCreated }: WorkspaceCreateButtonProps) {
  ...
  return (
    <>
      <button
        ...
        className="flex w-full items-center gap-2 rounded-lg px-2 py-2 text-left text-small font-medium outline-none hover:bg-sidebar focus-visible:outline focus-visible:outline-offset-2 focus-visible:outline-ink"
      >
        <Icon name="plus" className="size-4 shrink-0 text-text-secondary" />
        New workspace
      </button>
      <Dialog
        open={open}
        onClose={close}
        labelledBy="workspace-composer-title"
      ...
      >
        <form action={createNamedWorkspace}>
          <div className="flex h-14 items-center justify-between border-b border-border px-5">
            <h2 id="workspace-composer-title" className="text-h3">
              Create workspace
            </h2>
            <button
              type="button"
              onClick={close}
              aria-label="Close workspace composer"
              className="flex size-8 items-center justify-center rounded-lg text-text-muted outline-none hover:bg-surface-raised focus-visible:outline focus-visible:outline-offset-2 focus-visible:outline-ink"
            >
              <Icon name="close" />
            </button>
          </div>
