...
import {
  ...
  type PdfAutosaveSnapshot,
} from "@/lib/files/pdf-autosave";
...
import { describePdfOpenError } from "./pdf-document-content";
import { PdfImportedDocumentEditor } from "./pdf-imported-document-editor";
import {
  savePdfCopy,
  savePdfCopyToFiles,
  type PdfDownload,
  type PdfSaveCopyHandlers,
  type PdfSaveCopyInput,
} from "./pdf-save-copy";
import {
...
export type PdfFlushReason = "checkpoint" | "close";
export type PdfSaveCopyResult = "saved" | "downloaded" | "cancelled";

export type PdfCopyResult =
  | { kind: "notice"; message: string }
  | { kind: "error"; message: string };

export type PdfDocumentSaveState = {
  status: PdfAutosaveSnapshot["status"];
  error: string | null;
  version: number;
  savedLabel: string;
  savingLabel: string;
...
export type PdfDocumentEditorProps = {
  fileSourceId: string;
  fileName: string;
  initialBytes: Uint8Array;
  /** Markdown from panel `openPdfDocument` — thin pass-through to the shell. */
  ...
  onReload: () => void;
  onSaveStateChange?: (state: PdfDocumentSaveState) => void;
  onFlushReady?: (
    flush: ((reason: PdfFlushReason) => Promise<void>) | null,
  ) => void;
  onSaveCopyReady?: (handlers: PdfSaveCopyHandlers | null) => void;
};

export function PdfDocumentEditor(props: PdfDocumentEditorProps) {
  return (
    <PdfDocumentEditorSession
...
function downloadPdfCopy(copy: PdfDownload): void {
...
function PdfDocumentEditorSession({
  fileSourceId,
  fileName,
  initialBytes,
  initialMarkdown,
  ...
  const [serializer, setSerializer] = useState<PdfSerializer | null>(null);
  const [openError, setOpenError] = useState<string | null>(null);
  const [copyResult, setCopyResult] = useState<PdfCopyResult | null>(null);
    ...
    () =>
      createPdfAutosaveCoordinator({
        ...
        save: async (content, baseVersion, reason) => {
          try {
            ...
            if (
              ...
              const message =
                error instanceof LocalMirrorConflictError
                  ? "The original PDF changed outside Planevo. Reload it before editing further."
                  : "This PDF changed somewhere else. Reload the saved version before editing further.";
              throw new PdfAutosaveConflictError(message);
            }
            throw error;
          }
        },
        writeRecovery: async (draft) => {
          ...
          // the coordinator never replaces the original PDF without a draft.
  ...
  const [snapshot, setSnapshot] = useState<PdfAutosaveSnapshot>(() =>
  ...
  const flush = useCallback(
    async (reason: PdfFlushReason): Promise<void> => {
  ...
  const handleSerializerReady = useCallback(
    (nextSerializer: PdfSerializer | null) => {
  ...
  const performSaveCopy = useCallback(async (): Promise<PdfSaveCopyResult> => {
    setCopyResult(null);
    try {
  ...
  const saveCopyToFiles = useCallback(
    async (
      createInFiles: Parameters<PdfSaveCopyHandlers["saveToFiles"]>[0],
    ) => {
      setCopyResult(null);
      try {
  ...
  const saveCopyHandlers = useMemo<PdfSaveCopyHandlers>(
    () => ({
    ...
    >
          ...
          <span>
            {snapshot.error ??
              (snapshot.status === "conflict"
                ? "The saved PDF changed somewhere else."
                : "Planevo could not save this PDF.")}
          </span>
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
        <PdfImportedDocumentEditor
          ...
          onError={(error) => {
            setOpenError(describePdfOpenError(error));
          }}
          onSaveRequest={() =>
            void flush("checkpoint").catch(() => {
              refreshSnapshot();
...
export { pdfBytes } from "./pdf-document-content";
