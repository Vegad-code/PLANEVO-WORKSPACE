...
/**
 * Right-edge drag handle for the Files Library rail.
 * Drag resizes (persisted); click without drag collapses — same contract as
 * the primary workspace sidebar.
 */
export function LibraryResizeHandle({
  ...
  function handlePointerMove(event: React.PointerEvent<HTMLDivElement>) {
    ...
    const nextWidth = clampLibraryWidth(drag.current.startWidth + delta);
  ...
  return (
    <div
      ...
      aria-label="Resize library. Click to collapse."
      title="Drag to resize · click to collapse"
      onPointerDown={handlePointerDown}
      onPointerMove={handlePointerMove}
      onPointerUp={handleEndDrag}
      onPointerCancel={handleEndDrag}
      className="absolute inset-y-0 right-0 z-50 hidden w-1 cursor-col-resize touch-none bg-transparent transition-colors hover:bg-files-border-strong motion-reduce:transition-none lg:block"
    />
  );
}
