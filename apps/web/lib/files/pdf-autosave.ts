export class PdfAutosaveConflictError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "PdfAutosaveConflictError";
  }
}

export type PdfAutosaveStatus =
  | "saved"
  | "dirty"
  | "saving"
  | "conflict"
...
export type PdfAutosaveSnapshot = {
  status: PdfAutosaveStatus;
  error: string | null;
  version: number;
};

export type PdfRecoveryDraft = {
  fileSourceId: string;
  content: Uint8Array;
  baseVersion: number;
};

export type CreatePdfAutosaveCoordinatorArgs = {
  fileSourceId: string;
  ...
  writeRecovery: (draft: PdfRecoveryDraft) => Promise<void>;
  clearRecovery: () => Promise<void>;
};

export type PdfAutosaveCoordinator = {
  markChanged: () => void;
  flush: (reason: string) => Promise<void>;
  captureRecovery: () => Promise<void>;
  whenIdle: () => Promise<void>;
  getSnapshot: () => PdfAutosaveSnapshot;
};
...
function errorMessage(error: unknown): string {
  return error instanceof Error ? error.message : "Unable to save the PDF.";
}

/**
 * Coordinates browser-independent PDF serialization with durable recovery. Each source
 ...
 */
export function createPdfAutosaveCoordinator({
  fileSourceId,
  initialVersion,
  serialize,
  save,
  writeRecovery,
  clearRecovery,
}: CreatePdfAutosaveCoordinatorArgs): PdfAutosaveCoordinator {
  let version = initialVersion;
  let status: PdfAutosaveStatus = "saved";
  ...
  const getSnapshot = (): PdfAutosaveSnapshot => ({ status, error, version });
  ...
  const flushChanges = async (reason: string): Promise<void> => {
    while (savedGeneration < changedGeneration && status !== "conflict") {
      try {
        ...
        error = errorMessage(nextError);
        status =
          nextError instanceof PdfAutosaveConflictError ? "conflict" : "error";
