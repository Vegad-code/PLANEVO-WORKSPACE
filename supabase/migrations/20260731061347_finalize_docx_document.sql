-- DOCX saves use the existing staged-object compare-and-swap so a failed
-- concurrent save never overwrites the canonical OOXML object. The file source
-- remains the same Files entry; only its immutable backing version advances.
...
begin
  ...
  if p_format not in ('markdown', 'text', 'docx') then
