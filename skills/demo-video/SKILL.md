...
node skills/demo-video/record-flow.mjs \
  ...
  --out  /opt/cursor/artifacts/my-demo.mp4      # .mp4 → transcoded via ffmpeg; else .webm
```

Then reference it in your answer / PR body: `<video src="/opt/cursor/artifacts/my-demo.mp4" controls></video>`.
...
- Output is WebM natively; the harness transcodes to MP4 when `ffmpeg` is on PATH
  (MP4 plays inline in PR/GitHub; WebM is the fallback).
